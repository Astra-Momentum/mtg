...
def init():
    ...
    with open(CARD_BLACKLIST, "r") as blacklist_file:
    ...
    if not fs.exists(CARDS_JSON):
        with open(CARDS_JSON, "w") as cards_file:
            js.dump(js.loads(urlopen(
                Request(SCRYFALL_URL, headers=HTTP_HEADERS)).read()), cards_file, indent=4)
    ...
    with open(CARDS_JSON, "r+") as cards_file:
        ...
        temp_data = list(
            filter(lambda card: card[ID] not in BLACKLIST, temp_data))
        if initial_length != len(temp_data):
            was_modified = True
...
def fetch_card():
    for card in CARD_DATA:
        if IMAGES in card:
            generate_regular_image(
                card, TOKENS_CACHE if "Token" in card[TYPE] else REGULARS_CACHE)
        elif FACES in card:
            generate_double_image(card)
...
def generate_regular_image(card, card_dir: str):
    ...
    if not fs.exists(f"./{CARDS_CACHE}/{card_dir}/{card_name}/{card[ID]}.png"):
        try:
            Image.open(BytesIO(urlopen(Request(card[IMAGES][SMALL], headers=HTTP_HEADERS)).read())).save(
                f"./{CARDS_CACHE}/{card_dir}/{card_name}/{card[ID]}.png")
        except:
            print(f"Card {card[NAME]} could not be found.")
    ...
    if IMAGES in card[FACES][FRONT] and IMAGES in card[FACES][BACK]:
        ...
        for side, face in enumerate(card[FACES]):
            if not fs.exists(f"./{CARDS_CACHE}/{DOUBLES_CACHE}/{card_name}/{card[ID]}/{DOUBLE_CARD_NAMES[side]}.png"):
                try:
                    Image.open(BytesIO(urlopen(Request(face[IMAGES][SMALL], headers=HTTP_HEADERS)).read())).save(
                        f"./{CARDS_CACHE}/{DOUBLES_CACHE}/{card_name}/{card[ID]}/{DOUBLE_CARD_NAMES[side]}.png")
                except:
                    print(f"Card {card[NAME]} could not be found.")


def normalize(string): return re.sub(
    '[!@:."_?]', "", f"{string.__str__().replace(' ','')}").replace("//", "&&").replace("A-", "")
...
def append_deck_file(new_sub_deck: list[dict], deck_name: str, amount: str):
    with open(f"./{DECKS_CACHE}/{deck_name}.txt", "a") as deck_file:
        for card in new_sub_deck:
            ...
            deck_file.write(
                f"{card['front']}{CARD_SEPARATOR}{card_back}{amount}\n")
...
def parse_deck_file(deck: str) -> list[Card]:
    cards = []
    with open(f"./{DECKS_CACHE}/{deck}", "r") as deck_file:
        cards = [create_card_from_deck(line)
                 for line in deck_file.read().splitlines()]
    return cards
...
def create_decks() -> list[Deck]:
    decks = []
    for file in list_decks():
        decks.append(create_deck(file))
    return decks
...
def _get_simples(card_name: str) -> list[dict]:
    ...
    results = []
    if (fs.exists(root)):
        results = [_JSONCard(card_name, f"{root}/{id}", None)
...
def _get_doubles(card_name: str) -> list[dict]:
    ...
    results = []
    if (fs.exists(root)):
        results = [_JSONCard(card_name, f"{root}/{id}/{DOUBLE_CARD_NAMES[FRONT]}.png",
                             f"{root}/{id}/{DOUBLE_CARD_NAMES[BACK]}.png") for id in os.listdir(root)]
    return results
...
def search_cards_advanced(filters: dict) -> list[dict]:
    ...
    for non_token_card in list(filter(lambda card: TYPE not in card or "Token" not in card[TYPE], CARD_DATA)):
        if all(analyze_card_filter(non_token_card, key, filters[key]) for key in compared_keys):
            found_cards = _get_simples(
                non_token_card[NAME]) if IMAGES in non_token_card else _get_doubles(non_token_card[NAME])
...
def analyze_card_filter(card, expected_key: str, expected_value: str):
    if expected_key in card:
        return expected_value.lower() in (card[expected_key].__str__()).lower()
    if FACES in card:
        for side in card[FACES]:
            if expected_key in side:
                return expected_value.lower() in (side[expected_key].__str__()).lower()
    return False
...
def full_card_text(card):
    ...
    if FACES in card:
        for face in card[FACES]:
            full_text.join(["" if not TYPE in face else face[TYPE],
                           "" if not TEXT in face else face[TEXT]])
...
def newDeck():
    ...
    with open(f"./{DECKS_CACHE}/{name}.txt", "a") as deck_file:
        pass
    return name
...
def renameDeck(newName, oldName):
    os.rename(f"./{DECKS_CACHE}/{oldName}.txt",
              f"./{DECKS_CACHE}/{newName}.txt")


def getCardList(deck: str):
    cards = []
    with open(f"./{DECKS_CACHE}/{deck}", "r") as file:
        cards = [parse_card(line) for line in file.read().splitlines()]
...
def parse_card(cardString):
    ...
    back_image = None
    amount = parts[LAST]
    if len(parts) == DOUBLE_CARD_LENGTH:
        back_image = "../." + parts[1]
